...
const AdvisingStudent = ({ onLogout }) => {
  ...
  const { data, error, getData } = useData(`/api/advising/?id=${studentId}`);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      await getData(); // Fetch data from the advising endpoint
    };
    fetchData();
  }, [getData]);
  ...
  useEffect(() => {
    ...
    if (error) {
      triggerAlert("error", "Error", error?.data?.error || "Student not eligible for advising.");
  ...
  const handleAddToPending = () => {
    console.log("Defaults: ", defaultCourses);
  ...
  const addCourse = () => {
    const newCourse = suggestedCourses.length > 0 ? { ...suggestedCourses[0] } : {}; // Make a copy of the first eligible course or an empty object
    ...
    <div className="flex min-h-screen">
      ...
      >
        <div className="w-full max-w-[87.5rem] px-6">
          <h1 className="text-3xl font-semibold text-gray-800 mb-6">ADVISING STUDENT</h1>
              ...
              <tbody>
                {defaultCourses?.map((course, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    <td className="px-6 py-4 border-b">{course?.code || "N/A"}</td>
                    <td className="px-6 py-4 border-b">{course?.title || "N/A"}</td>
                    <td className="px-6 py-4 border-b">{course?.lecture_units || "N/A"}</td>
                    <td className="px-6 py-4 border-b">{course?.lab_units || "N/A"}</td>
                    <td className="px-6 py-4 border-b">{course?.lecture_hours || "N/A"}</td>
                    <td className="px-6 py-4 border-b">{course?.lab_hours || "N/A"}</td>
                    <td className="px-6 py-4 border-b">{course?.year_level || "N/A"}</td>
                    <td className="px-6 py-4 border-b">{course?.semester || "N/A"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex justify-end space-x-4">
            <button onClick={backToList} className="bg-gray-600 text-white px-4 py-2 rounded-[1.875rem] hover:bg-gray-700">
              Back to List
            </button>
            <button
